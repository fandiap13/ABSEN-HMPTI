<div class="modal fade" id="modaledit">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <?= form_open(base_url('admin/kegiatan/ubahKegiatan'), ['class' => 'form_edit']); ?>
      <div class="modal-body">
        <div class="form-group">
          <input type="hidden" name="id" value="<?= $kegiatan['id']; ?>">
          <label for="nama_kegiatan">Nama Kegiatan</label>
          <input type="text" name="nama_kegiatan" id="nama_kegiatan" class="form-control" value="<?= $kegiatan['nama_kegiatan']; ?>">
          <div class="invalid-feedback error_nama_kegiatan"></div>
        </div>
        <div class="form-group">
          <label for="tgl_kegiatan">Tanggal Kegiatan</label>
          <input type="datetime-local" name="tgl_kegiatan" id="tgl_kegiatan" class="form-control" value="<?= $kegiatan['tgl_kegiatan']; ?>">
          <div class="invalid-feedback error_tgl_kegiatan"></div>
        </div>
        <div class="form-group">
          <label for="keterangan">Keterangan</label>
          <textarea name="keterangan" id="keterangan" class="form-control" rows="3"><?= $kegiatan['keterangan']; ?></textarea>
          <div class="invalid-feedback error_keterangan"></div>
        </div>
      </div>
      <div class="modal-footer justify-content-between">
        <button type="button" class="btn btn-default" data-dismiss="modal">Tutup</button>
        <button type="submit" class="btn btn-primary btnUbah"><i class="fa fa-save"></i> Simpan perubahan</button>
      </div>
      <?= form_close(); ?>
    </div>
    <!-- /.modal-content -->
...
<script>
  ...
  $('.form_edit').submit(function(e) {
    ...
    $.ajax({
      ...
      dataType: "json",
      beforeSend: function() {
        $('.btnUbah').html('<i class="fa fa-spinner fa-spin"></i>');
        $('.btnUbah').attr('disabled', true);
      },
      complete: function() {
        $('.btnUbah').html('<i class="fa fa-save"></i> Simpan perubahan');
